...
impl McpProcess {
    fn start() -> Self {
        let mut command = Command::new(env!("CARGO_BIN_EXE_happy-wakey-mcp-server"));
        command
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::null());
        ...
        let mut child = command.spawn().expect("spawn happy-wakey MCP process");
...
fn stdio_wire_exposes_org_specific_client_and_provider_parity() {
    ...
    let initialized = mcp.request(
        ...
        &json!({
            ...
            "capabilities": {},
            "clientInfo": {"name": "happy-wakey-parity-test", "version": "1.0.0"},
        }),
    );
    assert_eq!(
    ...
    assert!(resources.iter().all(|resource| resource["uri"]
        .as_str()
        .is_some_and(|uri| uri.contains("happy-wakey"))));
    ...
    assert_eq!(identity["organization"], "happy-wakey");
    assert_eq!(
        identity["repository"],
    ...
    assert_eq!(posture["organization"], "happy-wakey");
